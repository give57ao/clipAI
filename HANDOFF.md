...
### 다음 작업 (Sonnet에 위임, 순서대로)
1. **오염된 hud_timeline JSON 재생성**: 어제 재스캔된 ~54개 GT 영상을 **단일
   프로세스**로 재스캔 (R10은 기본 꺼짐 그대로). `files/_obs_reprocess_todo.txt`와
   무관 — GT 61영상 목록 기준.
2. 재스캔 후 `_compare_hud_gt.py` + `_tp_diff.py --compare-to r8_final`로 클린
   측정 → TP 상실 0 확인되면 `--save-baseline r10_cleanbase`로 저장.
3. 클린 베이스라인 확보 후에만 R10(`verify_boundary_wins=True`) 재평가 논의.
