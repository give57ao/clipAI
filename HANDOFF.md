...
> ## ⏸ 재개 지점 (2026-07-14 컴퓨터 재시작으로 중단)
> **R7 = 폭0 FP 지배원인 "8 블라인드" 규명·수정. 코드·유닛테스트 완료·커밋됨,
> GT 96건 측정만 미완.** 다음 세션은 `HUD_ACE_HANDOFF.md` 맨 아래 "R7 재개 절차"의
> 3단계 명령(재계산→12개 재스캔→3종 측정)부터. 사용자 육안 검수로 폭0 오탐 9건이
> 전부 단일 원인(8을 못 읽어 7→10 가짜 올킬)임이 확정됨 — R6 "여러 원인 혼재"
> 결론을 뒤집는 발견. 재스캔 목록: `files/_r7_rescan_todo.txt`.

## ★ HUD 올킬 파이프라인 (2026-07-13 R6 완료 — recall 66.7%, precision 68.1%→68.8%)

**닉·스코어보드 없이** HUD K/D/A로 올킬 탐지. R5(2026-07-10)로 recall 66.7%
(64/96)/precision 71.1% 확보. R6(2026-07-13)는 폭0 FP(3킬이 전부 같은 시각) 분리
시도 — D채널·K밀도 로컬 신호 4종은 실측 기각, **상단 스코어 앵커 카드는 부분
성공**(위험 없는 소폭 개선, precision 71.1%→68.8%는 GT 96건 기준 재측정치로
71.1%와 직접 비교 시 표본 변동 있음 — 상세는 `HUD_ACE_HANDOFF.md` R6 절).

- **현재 성능(GT 96건)**: recall 66.7%(64/96) 유지, precision 68.8%(64/93).
  `_tp_diff --compare-to r5_final`: **유지 64, 상실 0** — 회귀 없음.
- **R6 핵심 발견** (상세: [`HUD_ACE_HANDOFF.md`](HUD_ACE_HANDOFF.md) §0 "R6" 절):
  1. **상단 라이브 HUD 팀 승수(`001|005|002`) 판독 성공** — "005" 박스를 풀프레임
     앵커로 템플릿 매칭해 `game_roi` ML crop의 프레임별 지터를 완전히 우회
     (8영상 전부 위치 표준편차 0.0px). `files/hud_score_wins.py` 신규.
  2. **GO/NO-GO 실측으로 명세의 가정이 틀렸음을 확인** — "라운드 안에 승수
     이벤트가 있냐"는 TP·FP가 안 갈림(둘 다 필연적으로 1개). **"2개 이상"**
     (라운드 병합 직접 증거)만 유의미했고, 이 조건은 폭0 FP 18건 중 **1건만**
     잡음 — 폭0 FP는 여러 원인이 섞인 카테고리라는 게 이번까지 3번째 확인.
  3. 게이트는 안전하게 좁혀서 채택(`_SCORE_SPLIT_MIN_EVENTS=2`,
     `detect_ace_hud.py`) — TP 손실 0 보장, FP 1건 감소.
- **코드**: 위 R5 코드 + `hud_score_wins.py`(신규, 앵커·이벤트), `detect_ace_hud.py`
  (게이트), `hud_from_cache.py`(score_win_events 연결), `hud_kda.py`(`_TOP_Y` 재보정)
- **하위 AI 작업 명세**: [`SONNET_TASK.md`](SONNET_TASK.md) R6 절 — **완료됨**.
  다음 카드는 HMM/전역 디코딩 또는 킬로그 카운팅(미착수, 사용자 제안).
...
- **다음 (남은 숙제)**:
  1. **E:\OBS 재처리 진행 중** — 75개 중 2026-07-13 세션에서 계속 처리 중
     (todo 리스트: `files/_obs_reprocess_todo.txt`, 백그라운드로 순차 진행).
  2. **폭0 FP 잔여 94%(17/18건) 미해결** — 로컬 임계값(K/D/K밀도/스코어 존재유무)
     4종 전부 약하거나 무의미했음이 확인됨. 다음은 HMM/전역 디코딩(§ "다음 큰
     카드") 또는 킬로그 줄 수 카운팅(2026-07-13 사용자 제안, 완전 미착수) 검토.
