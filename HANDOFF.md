...
- 현재 `compute_delta_kills`는 `delta >= 3`으로 ace 판정 → **ΔK=4,7,12,70 등 오탐 발생** (버그, 다음 수정 대상)
...
| 항목 | 결과 |
|------|------|
| `detect_rounds` | 스코어보드 **81**, 라운드 **82** (~31분) |
| 직접 매칭 | ~50/81 |
| 안전장치 `[T]` | **16** |
| `ace_rounds` (현 로직) | **12** — 일부 **오탐** |

올킬 후보 (현 `delta>=3` 로직, **재검증 필요**):

| 라운드 | 시각 | ΔK | 비고 |
|--------|------|-----|------|
| R03 | 3:37 | 3 | `[T]` 안전장치 |
| R17 | 18:53 | 4 | ⚠️ ΔK>3, `[T]` 팀원 `assacre` |
| R27 | 29:02 | 4 | ⚠️ ΔK>3 |
| R31 | 31:13 | 5 | ⚠️ ΔK>3 |
| R34 | 34:10 | 7 | ⚠️ ΔK>3 |
| R52 | 53:08 | 3 | 직접 ✓ |
| R61 | 61:21 | 5 | ⚠️ `[T]` 팀원 |
| R64 | 65:31 | 4 | ⚠️ ΔK>3 |
| R68 | 67:55 | 12 | ⚠️ K OCR 오류 |
| R72 | 72:04 | 70 | ⚠️ K OCR 오류 |
| R74 | 74:23 | 68 | ⚠️ K OCR 오류 |
| R75 | 75:30 | 4 | ⚠️ ΔK>3 |

출력: `E:\clipai_result\kill_timeline\2026-03-21 00-40-56_v1.json`

### 알려진 이슈 (다음 작업)

1. **ΔK 상한 미적용** — ace 판정을 `== 3`으로, `>= 4`는 `k_read_error` 플래그
2. **안전장치 오귀속** — 팀원 K를 본인으로 잡는 경우 (R17, R61 등) → 직접매칭 비중 더 올리기
3. **연속 라운드 읽기 실패** — R42~R48 (`consecutive_round_read_failures`)
4. **1c 미구현** — `ace_rounds` → ffmpeg 클립 자동 추출 없음
5. **리포트 시각** — `format_report`가 아직 초(s) 표기 → `M:SS` 변환 필요
