...
## 구현 중 수정 사항 (2026-07-31)

구현 과정에서 이 문서의 설계 일부가 실제와 다르게 조정됐다(둘 다 승인된
수정). 아래 두 항목만 실제 동작 기준이며, 나머지 문서 내용은 원래 설계
그대로 보존한다(무엇을 계획했고 무엇이 실제로 만들어졌는지 기록 목적):

- **인터프리터 경로**: 이 PC에는 프로젝트 전용 `C:\clipAI\.venv`가 없다.
  아래 본문의 `C:\clipAI\.venv\Scripts\python.exe` / "`.venv`를 subprocess로
  그대로 호출" 서술은 실제로는 이 PC의 **전역 Python 설치**
  (`C:\Users\give5\AppData\Local\Programs\Python\Python311\python.exe`,
  torch/opencv/easyocr가 직접 설치돼 있음)를 가리키는 것으로 대체됐다.
  `launcher.py`의 해당 상수명도 `VENV_PYTHON`이 아니라 `PIPELINE_PYTHON`이다.
- **결과 폴더 프롬프트 조건**: 아래 "② `launcher.py`" 5번 항목의 "종료
  코드와 무관하게(성공/실패 모두)" 서술은 실제 구현과 다르다. 실제로는
  파이프라인 종료 코드가 0이 아니면 실패 메시지만 출력하고 그 즉시
  반환하며(결과 폴더 프롬프트 생략), 종료 코드가 0일 때만 결과 폴더
  경로와 Y/N 프롬프트를 보여준다.

또한 `_wait_for_key()`는 `msvcrt.getch()`를 사용하므로 파이프/리다이렉트된
표준 입력에는 반응하지 않는다 — 즉 `echo | ...` 같은 방식으로 빌드된 exe의
완전 무인(headless) 실행을 스크립트로 흉내낼 수 없다. 실제 대화형 사용에는
영향 없지만, 이 exe를 대상으로 자동화된 스모크 테스트를 스크립트로 짜려는
사람은 알아둬야 한다.
