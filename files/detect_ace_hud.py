...
# R10(2026-07-15): 경계 자체의 진위를 승수(win) 이벤트로 교차검증.
# 실측 계기: 탭(Tab)으로 스코어보드를 잠깐 연 것도 row_miss run을 만들어 CNN
# 경계검증(R5)을 통과하는 경우가 있음(탭 화면이 진짜 스코어보드와 시각적으로
# 구분 안 됨) — 02-55-36 17:11.6, 22-46-39 41:06.9 실측 확인(사용자 육안: "탭을
# 잠깐 눌렀을 뿐, 게임은 안 끊기고 이어짐"). 이 경우 진짜 3킬 올킬이 두 라운드로
# 쪼개져 ΔK==3을 못 만듦(미탐 34건 중 최대 16건이 "인접 라운드 킬 합==3" 지문과
# 일치). 승수는 K채널과 독립된 신호라 이 오탐 유형에 원리적으로 안 걸림(라운드가
# 진짜 안 끝났으면 승수도 안 오름 — 실측: 두 사례 모두 경계 전후 20초간 승수 불변).
# 대조군(검증된 진짜 올킬 59건, score_win_events 있는 영상 전체)의 win 이벤트는
# round.end_sec 대비 -23.6s~+21.2s(대부분 -5~-6s, OCR 확정 지연) 범위 — 아래
# 마진은 이 분포를 넉넉히 감싸도록 설정(TP 손실 0 우선, 정밀도는 부차적).
_BOUNDARY_WIN_MARGIN_BEFORE = 28.0
_BOUNDARY_WIN_MARGIN_AFTER = 24.0
...
def _has_nearby_win_event(
    score_win_events: list[dict], t: float,
    before: float = _BOUNDARY_WIN_MARGIN_BEFORE, after: float = _BOUNDARY_WIN_MARGIN_AFTER,
) -> bool:
    """t 근방 [t-before, t+after]에 승수(win) 이벤트가 하나라도 있는지(R10)."""
    return any(
        e.get("kind") == "win" and (t - before) <= e.get("t_hi", -1e9) <= (t + after)
...
def scan_hud_aces(
    ...
    *,
    ...
    timeline = timeline_from_reads(
        ...
        boundary_score_gate=verify_boundary_wins,
    )
    if err:
...
def timeline_from_reads(
    ...
    *,
    ...
    R10(2026-07-15): 같은 score_win_events로 경계(boundary) 자체도 교차검증.
    CNN(R5)이 진짜 스코어보드로 오인하는 케이스(탭으로 스코어보드를 잠깐 연 것 —
    시각적으로 진짜와 구분 안 됨)를 승수 증가 유무로 걸러낸다. `_has_nearby_win_event`
    마진 안에 win 이벤트가 없으면 그 경계를 폐기(두 라운드 병합). score_win_events가
    비어있으면(앵커 탐지 실패 등) 이 게이트도 자동 비활성.
    안전핀: 연속 경계를 2개 이상 잇달아 기각하지 않는다(최초 실측 시 win 채널의
    ...
    for (start, last, _n) in rowmiss_runs(reads):
        ...
        mid = (start + last) / 2
        if _win_gate_active and not _has_nearby_win_event(score_win_events, mid) and not _prev_win_rejected:
            # R10 안전핀: 연속 경계를 두 개 이상 계속 폐기하지 않는다. 승수(win) 채널이
            # 구간적으로 통째로 끊기면(앵커는 잡히지만 특정 구간만 판독 실패) 그 구간의
            # 모든 진짜 경계가 연쇄로 기각돼 라운드가 수십 분 단위로 뭉개짐(실측:
            # 23-58-38 R36이 22분으로 병합돼 진짜 올킬 2건을 통째로 삼킴, 01-26-52
            # R80도 동일 패턴) — TP 손실 0 원칙 위반. 확정된 실패 사례(02-55-36,
            # 22-46-39)는 전부 "고립된 단발" 탭 가림이었으므로, 한 번 기각한 다음
            # 경계는 무조건 인정해 연쇄를 원천 차단한다(회수는 최대 2라운드 병합으로 제한).
            _prev_win_rejected = True
