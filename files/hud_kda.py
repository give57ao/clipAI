...
_CNN_V2_EIGHT = False   # 2026-07-23 Opus: R18까지 회귀 3원인 전부 해소 + 파일럿
                        # 검증 통과(순 recall +1, FP 0, 아래 코드는 확정). 전체
                        # 재스캔(~수십시간)은 사용자 결정으로 보류 — 이득 대비 비용
                        # 판단 후 나중에. 켤 준비는 완료(플래그만 True로 + 전체 재스캔).
                        # K 슬롯 CNN-8. R16(2026-07-23)으로 의미 변경: ON이어도 값을
